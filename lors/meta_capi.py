...
Настройка: META_DATASET_ID/META_CAPI_TOKEN из .env (см. config/settings.py;
старые имена META_PIXEL_ID/META_ACCESS_TOKEN читаются как фоллбэк — так
назывались до появления письменного ТЗ, внутри settings/этого модуля
остались как есть, чтобы не раздувать диф). Пока не заданы — send_to_meta()
тихо no-op'ится, эндпоинт всё равно отвечает 204 (фронту не с чем
разбираться). META_DATASET_ID здесь же используется как pixel id — в Meta
это один и тот же числовой идентификатор.
"""
