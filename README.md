...
- `POST /api/leads/` — публично, без авторизации. Единая точка входа для
  ...
  добавление пакета к заказу, если у модели его ещё нет (реальная позиция
  прайс-листа «كل أنواع الباكاج», подтверждена заказчиком скриншотом
  таблицы; изначально была ошибочно удалена как «несуществующая» —
  восстановлена в `lors/migrations/0025_restore_package_dese_fix_carnival_category.py`,
  см. её докстринг). Аналогично `Lead.dese` — реальная платная опция
