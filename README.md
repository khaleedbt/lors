...
- `GET /api/settings/` — публично, без списка/id. Единая запись настроек
  ...
  типа, с необязательной подписью). Редактируется только в `/admin/`
...
- `GET /api/materials/` и `GET /api/colors/` — публично, только чтение,
  только активные (`is_active=True`) записи. Справочники для конфигуратора
  коврика: материал (`Material` — EVA локальный/импортный, ковролин
  10/20мм, заготовлены в `lors/migrations/0014_seed_materials.py`) и цвет
  (`Color`, с HEX для превью), общая палитра для коврика/окантовки/пятки.
  `GET /api/car-models/<id>/` дополнительно отдаёт `mat_prices` — цену
  полного комплекта для этой модели по каждому материалу (матрица цен
  «модель × материал», модель `MatSetPrice`, редактируется инлайном на
  карточке модели в `/admin/`; если для модели ничего не заведено —
  список пуст, конфигуратор на фронте должен в этом случае предлагать
  написать менеджеру, а не показывать цену).
