...
- **Throttling** (`config/settings.py` → `REST_FRAMEWORK`): общий лимит
  ...
  см. `get_throttles()` в `lors/views.py`. Общий файловый кэш используется
  всеми локальными воркерами; для нескольких серверов нужен общий Redis
  (CACHE_BACKEND/CACHE_LOCATION и установленный клиент redis).
  NUM_PROXIES=1 соответствует одному доверенному nginx, который перезаписывает X-Forwarded-For.
...
- Загрузка фото (`Lead.uploaded_photos`, `Review.photo`) — throttling не
  ...
  `lors/serializers.py`).
- `POST /api/meta-event/` — ограничение IP через общий кэш с TTL.
  Отправка использует четыре потока и максимум 16 задач на процесс;
  при переполнении событие отбрасывается. Публичный Purchase запрещён.
...
- **Origin-проверка** (`lors/permissions.py` → `OriginAllowed`/
  `is_allowed_origin`) на `POST /api/leads/`, `/api/reviews/` (403, если
  Origin/Referer не из `CORS_ALLOWED_ORIGINS`) и `/api/meta-event/` (не
  403 — там любой некорректный ввод молча не обрабатывается, см. её
  докстринг). Важно понимать предел этой меры: Origin — обычный
  заголовок, целенаправленная атака легко его подделает под lorssy.com
  — это НЕ полноценный контроль доступа «только фронтенду», а фильтр
  от generic-спам-ботов, которые долбят найденный POST-эндпоинт без
  Origin вообще или с чужого домена. Настоящий API-ключ тут в принципе
  не работает (в отличие от `/api/assistant/message/`, где бот — не
  браузер): любой секрет во фронтенд-коде виден в DevTools → Network
  любому посетителю сайта.
- GET-эндпоинты (каталог, цены, товары) сознательно оставлены полностью
  открытыми, без Origin-проверки — это публичные данные сайта, а не
  что-то, что нужно прятать; настоящая защита от скрейпинга — это
  Cloudflare/WAF на уровне сервера, не задача уровня Django.
...
`body_variant` (распознаётся этой командой из `name`, где кузов записан
по-русски — но **сохраняется уже переведённым на арабский**, см. таблицу
терминов ниже; сайт и бот только на арабском, русский текст клиенту
попадать не должен) — оба сейчас покрывают меньшинство каталога,
показываются вместе одним списком, с запасным пунктом на случай
отсутствия обоих.

Таблица перевода (`BODY_VARIANT_MAP` в `parse_car_models.py`) — что
именно ищется в `name` и что в итоге сохраняется в `body_variant`:

| В `name` (рус.) | `body_variant` (араб.) |
|---|---|
| Седан | سيدان |
| Хэтчбек | هاتشباك |
...
«Рестайлинг»/«Дорестайлинг» (пре-/пост-фейслифт) в `name` тоже
встречаются, но не переводятся — команда их просто вырезает при сборке
`base_model`, никуда не сохраняет (различие поколений до/после рестайлинга
теряется, это осознанный компромисс, не баг). «Long»/«Sportback» и другие
нераспознанные слова остаются как есть внутри `base_model`.

Сам ассистент (`assistant/ai_tools.py`, `_car_display_name`) тоже берёт
`base_model`/`body_variant`/`year_from`/`year_to` вместо сырого `name` при
формировании ответа ИИ на инструменты поиска — иначе русские слова из
`name` (то самое «Дорестайлинг») попадали бы в контекст модели, и не
показать их клиенту зависело бы только от того, послушается ли она
текстовой инструкции в промпте, а не гарантировалось бы структурой данных.
...
валидируется `MetaEventSerializer` (`lors/serializers.py`), не ручными
`dict.get()` проверками. `user` (`phone`/`email`/`firstName`/... —
опционально, что успело собраться на фронте к моменту события) хешируется
SHA-256 после нормализации (телефон — только цифры с кодом страны,
`0912345678` → `963912345678`; остальное — `trim().lower()`); пустые поля
не отправляются вовсе (иначе падает Event Match Quality события).
`fbp`/`fbc` — из cookies браузера (не хешируются, это не PII).

`eventName` — только из белого списка (`PageView`/`ViewContent`/`Search`/
`Lead`/`Contact`/`AddToCart`/`CustomizeProduct`/`InitiateCheckout`/
`CompleteRegistration`) — эндпоинт публичный, без списка в датасет можно
было бы залить что угодно. **`Purchase` сознательно исключён** из
белого списка (в отличие от исходного ТЗ, где он в списке есть) — это
единственное событие, где `customData.value` реально должен быть суммой
настоящей оплаты; публичный неаутентифицированный клиент мог бы прислать
`Purchase` с любой вымышленной суммой и засорить аналитику конверсий
(конкретно этот вектор проверен вручную — событие с суммой 99999 доходило
до отправителя до фикса). Если понадобится реальный `Purchase` — генерировать
его на сервере после подтверждённого бизнес-события (оплата/статус лида),
не принимать от клиента. `eventId` короче 8 символов — тоже отбрасывается,
лучше потерять сигнал, чем задвоить лид без валидного ключа дедупликации.
Ответ — всегда `204`, включая некорректный JSON в теле, чужой `Origin`
(см. `OriginAllowed`/`is_allowed_origin` в разделе «Защита от спама» выше)
и ошибки самой Meta: фронт разбирать детали не будет (`.catch(() => {})`),
а шум в консоли пользователя пользы не даёт. Битый JSON — отдельный
нюанс: `request.data` кидает `ParseError` при обращении, а не заранее,
поэтому `MetaEventView.post()` явно оборачивает чтение тела в
`try/except`, не полагаясь на `hasattr()` (тот его не ловит — исключение
не `AttributeError`, атрибут `data` у DRF `Request` есть всегда).

Отправка в Meta — через общий `ThreadPoolExecutor` (4 воркера) с
ограничением на 16 событий в очереди одновременно (`BoundedSemaphore`) —
не отдельный `threading.Thread` на каждый запрос без ограничения (старая
версия могла расти неограниченно при потоке публичных запросов); очередь
переполнена — событие тихо теряется, не блокирует ответ клиенту. Ретраи
на отправке — 2 (300мс/600мс), только на 5xx/429; 4xx — ошибка в данных,
ретраить бессмысленно.

Rate-limit — 60 запросов/мин на IP, через общий кэш Django (`CACHES` в
`config/settings.py`, по умолчанию `FileBasedCache` в `.cache/django/` —
переживает рестарт процесса и общий для всех воркеров gunicorn, в
отличие от прежнего in-memory `dict`, который не шарился между
воркерами). IP определяется через `NUM_PROXIES` (см. ниже) — без него
`X-Forwarded-For` от **клиента** мог бы подделываться, полностью обходя
лимит (проверено: 6 запросов с 6 разными поддельными префиксами XFF —
все проходили как разные IP, `test_forwarded_prefix_does_not_reset_throttle`
в `lors/tests.py`).

Настройка через `.env`: `META_DATASET_ID`, `META_CAPI_TOKEN` (получаются в
...
проекте; `META_DATASET_ID` используется и как dataset id, и исторически
называлось Pixel ID — в Meta это один и тот же числовой идентификатор).
Старые имена `META_PIXEL_ID`/`META_ACCESS_TOKEN` (из первой реализации, до
появления письменного ТЗ) по-прежнему читаются как фоллбэк — если что-то
уже настроено под старыми именами, менять не обязательно, но для новых
развёртываний использовать канонические. Пока не задано ни то, ни другое —
интеграция тихо no-op'ится, эндпоинт всё равно отвечает `204`.
`META_TEST_EVENT_CODE` — опционально, для проверки в разделе «Тестирование
событий» перед боевым запуском, в проде должен быть пуст. `NUM_PROXIES` —
число доверенных прокси перед Django (1 — сам nginx на проде; 0 — прямые
локальные запросы без прокси), используется и DRF-throttling, и
`meta_capi.get_client_ip()` (через `rest_framework.throttling.BaseThrottle`)
как единственный источник правды для клиентского IP.

Фото заявок (`Lead.uploaded_photos`) отдаются не напрямую nginx, а через
`GET /media/leads/<path>` (`LeadPhotoView`, только для staff) — раньше вся
`/media/` раздавалась статикой без проверки прав, и фото жалобы/заказа с
именем/телефоном клиента были доступны кому угодно по прямой ссылке.
nginx-конфиг (`deploy/nginx_lorssy.com.conf`) поэтому проксирует именно
`/media/leads/` в Django, а не отдаёт как статику (`location ^~
/media/leads/`), остальной `/media/` (товары, опубликованные отзывы)
по-прежнему отдаёт nginx напрямую — это осознанно публичные картинки.
...
### Применение исправлений безопасности (17.09.2026)

- SECRET_KEY обязателен: случайная строка не короче 50 символов; DEBUG по умолчанию False. Не меняйте действующий ключ без планирования сброса сессий.
- Примените правила из `deploy/nginx_lorssy.com.conf` к активному HTTPS virtual host: перезапись X-Forwarded-For, ограничение запросов, проксирование `/media/leads/` в Django. Проверьте `nginx -t` перед перезагрузкой. Файл содержит limit_req_zone, который должен находиться в контексте http и объявляться один раз.
- Фотографии заявок теперь требуют staff-сессии или staff Basic Auth; файлы остаются на прежних местах. Публичная раздача `/media/leads/` в старом nginx должна быть отключена при развёртывании. CDN/прокси не должны кэшировать эти ответы; ранее сохранённые публичные копии следует очистить.
- Публичные ответы каталога больше не включают template_code, notes и sheet_row.
- Дампы исключены из текущего индекса Git и игнорируются. Старые коммиты продолжают содержать дампы; очистка истории, отзыв сессий и смена затронутых паролей выполняются отдельно при подтверждённой необходимости.
- nginx здесь показан для HTTP; существующие настройки TLS сохраняйте. HSTS включайте после проверки HTTPS.
